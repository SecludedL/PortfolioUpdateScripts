...
export class DataRetrieverFX extends DataRetrieverAbstract {
  ...
  public getLatestDetails(ticker: string): Instrument {
    ...
    var baseCurrency = matches[1];
    var pairCurrency = matches[2];
    var updatedQuote = this.getLatestFxQuote(baseCurrency, pairCurrency);
  ...
  private getLatestFxQuote(baseCurrency, pairCurrency: string) {    
    ...
    if (responseObj.quotes != null) {
      if (baseCurrency !== this.fixedBaseCurrency) {
        let baseRate = responseObj.quotes[this.fixedBaseCurrency + baseCurrency];
        let pairRate = responseObj.quotes[this.fixedBaseCurrency + pairCurrency];
